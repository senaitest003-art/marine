...
export interface Prices{eurT:Record<Year,number>;usdT:Record<Year,number>;eurGJ:Record<Year,number>}
export interface Fuel {id:FuelId;name:string;lcv:number;ci:number;wtt:number;co2:number;ch4:number;n2o:number;slip:number;price:Prices;priceBasis:FuelPriceBasis;rfnbo:boolean;bio:boolean;food:boolean;eligible:boolean;maxSupply:number;maxBlend:number;minBlend:number;pilotShare:number;pilotFuel:FuelId;blue?:{upstream:number;production:number;capture:number;residual:number;leakage:number;transport:number;advanced:boolean}}
