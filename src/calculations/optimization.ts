...
import {fuelCost} from './fuelCost';import {ttwEmissions} from './euETS';import {blendCI,complianceBalance,penalty,targetCI} from './fuelEU';import {annualizedCapex,initialConversionCapex} from './capex';import {dedicatedSelection} from './pooling';import {priceSeries} from './fuelPricing';import {fuelAvailabilityLimit,pathwayAvailable,vesselCanUseFuel} from './fuelConstraints';
...
const usable=(v:Vessel,f:Fuel,share:number,y:Year)=>vesselCanUseFuel(v,f)&&pathwayAvailable(f,y)&&share<=Math.min(v.maxBlend,f.maxBlend);
export const option=(strategy:DeploymentStrategy,fleet:Vessel[],base:Fuel,alt:Fuel,share:number,dedicated:number,y:Year,s:Settings):Option=>{
 ...
 for(const v of fleet){let x=selected?(selected.has(v.id)?1:0):share;if(x>0&&!usable(v,alt,x,y)){technicalLimit=true;x=0}if(x>0)users.push(v);
 ...
 if(cleanEnergyGJ>fleet.reduce((sum,v)=>sum+v.energyGJ,0)*fuelAvailabilityLimit(alt,y)+1e-8)technicalLimit=true;
