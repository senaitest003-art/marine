...
import {fuelCost} from './fuelCost';import {ttwEmissions} from './euETS';import {blendCI,complianceBalance,penalty,targetCI} from './fuelEU';import {annualizedCapex} from './capex';import {dedicatedSelection} from './pooling';import {priceSeries} from './fuelPricing';
...
export interface Option extends CostResult{
 ...
 fuelEUPenalty:number;annualizedCapex:number;externalCreditCost:number;totalComplianceCost:number;
...
export const option=(strategy:DeploymentStrategy,fleet:Vessel[],base:Fuel,alt:Fuel,share:number,dedicated:number,y:Year,s:Settings):Option=>{
 ...
 let fc=0,cleanEnergyGJ=0,emissions=0,balance=s.bankedSurplus+s.borrowedBalance,altDemand=0,baseDemand=0,ciWeighted=0,scopedEnergy=0,users=0,technicalLimit=false;
 ...
 const cleanFuelCost=cleanEnergyGJ*priceSeries(alt.price,s.scenario).eurGJ[y],cleanVesselCount=selected?.size||(share>0?users:0);
 return{fuelId:alt.id,fuelName:alt.name,strategy,blendShare:dedicated?null:share,cleanVesselCount,cleanFuelDemandTonnes:altDemand,bunkerDemandTonnes:baseDemand,fuelCost:fc,cleanFuelCost,etsCost:ets,fuelEUPenalty:pen,annualizedCapex:capex,externalCreditCost:0,totalComplianceCost:total,label:strategy,share,dedicated:selected?.size||0,fuel:fc,ets,penalty:pen,capex,externalCredit:0,other,total,balance,actualCI,emissions,altDemand,baseDemand,technicalLimit};
};
