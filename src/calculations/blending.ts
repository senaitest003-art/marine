...
export const minimumBlend=(base:Fuel,alt:Fuel,year:Year,s:Settings,energyGJ:number,slip?:number):BlendResult=>{
 ...
 const alternative=effectiveCI(alt,year,s,slip);
 ...
 const denominator=bunker-target+factor*(target-alternative);
 ...
 if(raw>max){const reason=`Required blend ${(raw*100).toFixed(1)}% exceeds technical maximum blend ${(max*100).toFixed(1)}%`;return failed({...baseDetails,actualCI:blendCI(base,alt,max,year,s,slip),reason},raw,energyToMass(energyGJ*(1-max),base.lcv),energyToMass(energyGJ*max,alt.lcv));}

 const share=Math.max(raw,Math.min(max,Math.max(0,alt.minBlend)));
 const altTonnes=energyToMass(energyGJ*share,alt.lcv);
 const baseTonnes=energyToMass(energyGJ*(1-share),base.lcv);
