...
export const effectiveCI=(f:Fuel,_year:Year,s:Settings,slip?:number)=>{
 ...
 // adjust only the difference from that reference.  Adding the full engine
 // slip here double-counted methane and turned the default LNG CI of 75 into
 // ~95 gCO2e/MJ, incorrectly making LNG non-compliant in 2030.
 const selectedSlip=slip??f.slip;
 return base+(selectedSlip-f.slip)*1000*s.gwpCH4/f.lcv;
};
...
export const blendCI=(base:Fuel,alt:Fuel,share:number,year:Year,s:Settings,slip?:number)=>{const r=reward(alt,year,s),pilot=alt.pilotShare,cleanShare=share*(1-pilot),baseShare=1-cleanShare;return (effectiveCI(base,year,s)*baseShare+effectiveCI(alt,year,s,slip)*cleanShare)/(baseShare+cleanShare*r)};
